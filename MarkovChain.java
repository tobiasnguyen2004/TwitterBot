...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        if (!chain.containsKey(first)) {
            ProbabilityDistribution<String> n = new ProbabilityDistribution<>();
            n.record(second);
            chain.put(first, n);
        } else {
    ...
    public boolean hasNext() {
        if (currentString.equals(END_TOKEN)) {
            return false;
        }
        return true;
    }
    ...
    public String next() {
        ...
        if (chain.containsKey(currentString)) {
            currentString = chain.get(currentString).pick(ng);
        } else {
            currentString = END_TOKEN;
        }
        return s;
    }
