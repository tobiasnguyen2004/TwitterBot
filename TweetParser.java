...
public class TweetParser {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (fli.hasNext()) {
            String column = extractColumn(fli.next(), tweetColumn);
            if (column != null) {
                tweets.add(column);
            }
        }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        int l = wordsList.length;
        for (int x = 0; x < l; x++) {
            if (cleanWord(wordsList[x]) != null) {
                cleaned.add(cleanWord(wordsList[x]));
            }
        }
        return cleaned;
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<String> sentences = tweetSplit(removeURLs(tweet));
        for (String sentence : sentences) {
            List<String> cleaned = parseAndCleanSentence(sentence);
            if (cleaned.size() > 0) {
                sentencesList.add(cleaned);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<String> tweetList = csvDataToTweets(br, tweetColumn);
        for (String tweet : tweetList) {
            if (tweet != null) {
                trainingSet.addAll(parseAndCleanTweet(tweet));
            }
        }
        return trainingSet;
